...
using SillyMIDI.Core.Utilities;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    private static readonly string BaseAppDataPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "SillyMIDI");
...
#pragma warning disable CRR0026 // Unused member
    public static Bitmap FallbackImage
    {
        get
        {
            return fallbackImage ??= new Bitmap(AssetLoader.Open(new Uri("avares://SillyMIDI.Core/Assets/cover_dark.png")));
        }
    }
...
#pragma warning restore CS8618
    ...
    private static void RegisterSetupLogger(ContainerBuilder builder)
    {
        ...
        _logger = _loggerFactory.CreateLogger("SillyMIDI.Core");
    ...
    private static void ObtainLock()
    {
        ...
        try
        {
            var lockPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SillyMIDI", "SillyMIDI.lock");
            if (!SingleInstanceLock.TryAcquire(lockPath, out _singleInstanceLock))
            {
                Console.WriteLine("SillyMIDI is already running. Only one instance is allowed.");
                Environment.Exit(1);
                return;
            }
        }
    ...
    {
        //Make sure only one copy of SillyMIDI is running
        ObtainLock();
