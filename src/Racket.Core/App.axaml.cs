...
using Racket.Core.Utilities;
using Racket.Core.ViewModels;
using Racket.Core.ViewModels.Players;
using Racket.Core.Views;
using WateryTart.MusicAssistant;
using ColourService = Racket.Core.Services.ColourService;
using PlayersService = Racket.Core.Services.PlayersService;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    private static readonly string BaseAppDataPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Racket");
...
#pragma warning disable CRR0026 // Unused member
    public static Bitmap FallbackImage
    {
        get
        {
            return fallbackImage ??= new Bitmap(AssetLoader.Open(new Uri("avares://Racket.Core/Assets/cover_dark.png")));
        }
    }
...
#pragma warning restore CS8618
    ...
    private static void RegisterSetupLogger(ContainerBuilder builder)
    {
        ...
        _logger = _loggerFactory.CreateLogger("Racket.Core");
    ...
    private static void ObtainLock()
    {
        ...
        try
        {
            var lockPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Racket", "Racket.lock");
            if (!SingleInstanceLock.TryAcquire(lockPath, out _singleInstanceLock))
            {
                Console.WriteLine("Racket is already running. Only one instance is allowed.");
                Environment.Exit(1);
                return;
            }
        }
    ...
    {
        //Make sure only one copy of Racket is running
        ObtainLock();
